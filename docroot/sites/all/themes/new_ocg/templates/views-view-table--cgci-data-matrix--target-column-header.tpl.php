...
if (!is_numeric($column4)) {
  ?>
  <td class='outer colN' headers="<?php print $rows[$column4]['item_id_1']; ?>-rh seq_ch <?php print $rows[$column4]['nid']; ?>-bn">
    <div class="cellheader"></div>
    <div class='cellbody'></div>
  </td>
<?php } else { ?>
  <?php if ($rows[$column4]['field_availability'] == 'Unavailable'): ?>
    <?php $header = '<span' . $rows[$column4]['field_link'] . '>' . $rows[$column4]['field_link'] . '</span><br />'; ?>
  <?php endif; ?>
  <?php if ($rows[$column4]['field_availability'] == 'Protected'): ?>
    <?php $header = '<a class="protected" target="_blank"' . $rows[$column4]['field_link'] . ' href="' . $rows[$column4]['field_link_1'] . '">' . $rows[$column4]['field_link'] . '</a><br />'; ?>
  <?php endif; ?>
  <?php if ($rows[$column4]['field_availability'] == 'Public'): ?>
    <?php $header = '<a class="public" target="_blank"' . $rows[$column4]['field_link'] . ' href="' . $rows[$column4]['field_link_1'] . '">' . $rows[$column4]['field_link'] . '</a><br />'; ?>
  <?php endif; ?>
  <td class='outer colN' headers="<?php print $rows[$column4]['item_id_1']; ?>-rh seq_ch <?php print $rows[$column4]['nid']; ?>-bn">
    <table class="cell">
      <tbody>
