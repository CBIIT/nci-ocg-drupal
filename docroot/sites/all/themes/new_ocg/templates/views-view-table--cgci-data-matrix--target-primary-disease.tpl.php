...
?><?php print $attributes; ?> width="100%">
  ...
  <tbody>
    ...
    <?php } ?>
    <?php if ($disease->field_methylation->value()) { ?>
      <td class='outer colW' headers="<?php print $disease->item_id->value(); ?>-rh cna_ch <?php print $node; ?>-bn">
        <table class="cell">
          <tbody>
            <?php foreach ($disease->field_methylation->field_column_header as $methylation): ?>
              <tr>
                <th id="<?php print $disease->item_id->value(); ?>_cna" scope="col" class="cellheader">
                  <?php if (!empty($methylation->field_link->title->value())): ?>
                          <?php if ($methylation->field_availability->value() == 'Unavailable'): ?>
                            <div class="cellheader"><?php print $methylation->field_link->title->value(); ?></div>
                          <?php endif; ?>
                        <?php endif; ?>
                        <?php if (!empty($methylation->field_link->title->value()) && !empty($methylation->field_link->url->value())): ?>
                          <?php if ($methylation->field_availability->value() == 'Protected'): ?>
                            <div class="cellheader"><a class="protected" target="_blank" <?php print $link_title; ?> href='<?php print $methylation->field_link->url->value(); ?>'><?php print $methylation->field_link->title->value() . '†'; ?></a></div>
                          <?php endif; ?>
                          <?php if ($methylation->field_availability->value() == 'Public'): ?>
                            <div class="cellheader"><a class="public" target="_blank" <?php print $link_title; ?> href='<?php print $methylation->field_link->url->value(); ?>'><?php print $methylation->field_link->title->value(); ?></a></div>
                          <?php endif; ?>
                        <?php endif; ?>
          </th>
        </tr>
        <tr>
          <td class="cellbody" headers="<?php print $disease->item_id->value(); ?>_rh cna_ch <?php print $node; ?>_bn <?php print $disease->item_id->value(); ?>_cna">
            <div class='cellbody'>
              <?php foreach ($methylation->field_column_values as $i => $column_values): ?>
                <?php $field_link = field_view_field('field_collection_item', entity_load_single('field_collection_item', $column_values->item_id->value()), 'field_link'); ?>
                <?php
                if (!empty($field_link['#items'][0]['attributes']['title'])) {
                  $link_title = 'title="' . $field_link['#items'][0]['attributes']['title'] . '"';
    ...
    <?php } else { ?>
      <td class='outer colN' headers="<?php print $disease->item_id->value(); ?>-rh meth_ch <?php print $node; ?>-bn"><div class="cellheader"></div></td>
    <?php } ?>
